...
> Kode ini diimpor dari repo `pactio` (nama internal/lama proyek — package Android dan
> beberapa identifier kode masih memakai nama `Pactio`/`keluargakendali`, belum di-rename
> penuh). Nama produk yang tampil ke pengguna sudah "TimeCraft" di seluruh UI Android dan
> web dashboard.
...
| Bagian | Status |
|---|---|
| Backend (`server.js`) | Berjalan & teruji (`npm test` lulus) |
...
- Android tidak pernah menyimpan token/kata sandi/PIN sebagai teks biasa, dan tidak pernah
  mencetaknya ke log.
- API produksi wajib HTTPS — tidak ada cleartext HTTP.
